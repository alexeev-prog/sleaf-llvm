...
auto Parser::synchronize() -> void {
    m_panic_mode = false;
    synchronize_after_error({TokenType::FUNC, TokenType::VAR, TokenType::CONST,
...
auto Parser::declaration() -> std::unique_ptr<Stmt> {
    try {
        if (match(TokenType::FUNC)) return function_decl();
        if (match(TokenType::VAR)) return var_declaration(false);
        if (match(TokenType::CONST)) return var_declaration(true);
        return statement();
...
auto Parser::comparison() -> std::unique_ptr<Expr> {
    ...
    while (match_any({TokenType::LESS, TokenType::LESS_EQUAL,
...
auto Parser::type_annotation() -> TokenType {
    static const std::unordered_map<std::string, TokenType> type_map = {
        ...
        {"string", TokenType::STRING}, {"char", TokenType::CHAR}, {"void", TokenType::VOID}
    };
